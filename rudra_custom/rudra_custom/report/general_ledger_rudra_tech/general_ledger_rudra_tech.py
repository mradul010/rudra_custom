...
from frappe.utils import cstr, date_diff, flt, getdate, today
...
def get_result(filters, account_details):
	...
	enrich_invoice_details(gl_entries, filters)
...
def enrich_invoice_details(gl_entries, filters):
	"""Bulk fetch invoice metadata and add it to invoice-related GL rows."""
	sales_invoice_names = set()
	...
	for gle in gl_entries:
		...
		if voucher_type == "Sales Invoice" and voucher_no:
			sales_invoice_names.add(voucher_no)
		elif voucher_type == "Purchase Invoice" and voucher_no:
			purchase_invoice_names.add(voucher_no)

		if against_voucher_type == "Sales Invoice" and against_voucher:
			sales_invoice_names.update(get_voucher_names(against_voucher))
		elif against_voucher_type == "Purchase Invoice" and against_voucher:
			purchase_invoice_names.update(get_voucher_names(against_voucher))
	...
	if sales_invoice_names:
		...
		sales_orders_by_invoice = {name: [] for name in sales_invoice_names}
		for item in frappe.get_all(
			"Sales Invoice Item",
			filters={"parent": ("in", list(sales_invoice_names))},
			fields=["parent", "sales_order"],
			order_by="idx",
		):
			if item.sales_order and item.sales_order not in sales_orders_by_invoice[item.parent]:
				sales_orders_by_invoice[item.parent].append(item.sales_order)

		for invoice_name, invoice in sales_invoice_map.items():
			invoice.sales_order_no = ", ".join(sales_orders_by_invoice.get(invoice_name, []))
	...
	for gle in gl_entries:
		...
		if voucher_type not in ("Sales Invoice", "Purchase Invoice"):
			voucher_type = gle.get("against_voucher_type")
			invoice_names = get_voucher_names(gle.get("against_voucher"))
		elif voucher_no:
			invoice_names = [voucher_no]

		if voucher_type == "Sales Invoice":
			invoices = [sales_invoice_map[name] for name in invoice_names if name in sales_invoice_map]
			if invoices:
				gle["sales_invoice_number"] = ", ".join(invoice.name for invoice in invoices)
				gle["customer_purchase_order"] = ", ".join(
					deduplicate_values(invoice.po_no for invoice in invoices)
				)
				gle["sales_order_no"] = ", ".join(
					deduplicate_values(invoice.sales_order_no for invoice in invoices)
				)
				gle["due_date"] = min(
			...
			if invoices:
				gle["purchase_invoice_number"] = ", ".join(invoice.name for invoice in invoices)
				gle["due_date"] = min(
					(invoice.due_date for invoice in invoices if invoice.due_date),
					default=None,
				)
...
def get_due_days(gle, current_date):
	...
	due_date = getdate(gle.get("due_date")) if gle.get("due_date") else None

	if due_date and posting_date <= due_date:
		return date_diff(due_date, posting_date)

	return date_diff(current_date, posting_date)


def get_voucher_names(voucher_no):
	if not voucher_no:
		return []

	return [name.strip() for name in cstr(voucher_no).split(",") if name.strip()]
...
def get_columns(filters):
	...
	columns += [
		...
		{
			"label": _("Interest / Day"),
			"fieldname": "interest_per_day",
			"fieldtype": "Currency",
			"options": "presentation_currency",
			"width": 120,
		},
